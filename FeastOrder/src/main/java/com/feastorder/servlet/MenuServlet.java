...
// Loads menu items (all, filtered by category, or a single item for the detail view) and forwards to menu.jsp
@WebServlet("/menu")
public class MenuServlet extends HttpServlet {
    ...
    // Loads menu items (optionally filtered by category) and forwards to menu.jsp.
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // itemId takes priority, send links of "View Details", and menu.jsp only shows the detail view when "item" is set
            if (itemIdParam != null && !itemIdParam.isBlank()) {
                ...
                // the category filter tabs render correctly, when navigates back to the grid from here
