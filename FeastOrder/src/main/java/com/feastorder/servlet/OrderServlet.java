...
public class OrderServlet extends HttpServlet {
    ...
    // Turns the session cart into a persisted order and shows confirmation.
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!"Pickup".equals(orderType) && !"Delivery".equals(orderType)) {
            request.setAttribute("error", "Please select an order type.");
            request.getRequestDispatcher("/WEB-INF/jsp/user/cart.jsp").forward(request, response);
            return;
        }
        if ("Delivery".equals(orderType) && (deliveryAddress == null || deliveryAddress.trim().length() < 10)) {
            request.setAttribute("error", "Please enter a complete delivery address.");
            request.getRequestDispatcher("/WEB-INF/jsp/user/cart.jsp").forward(request, response);
            return;
        }
        ...
        order.setDeliveryAddress("Delivery".equals(orderType) ? deliveryAddress.trim() : null);
