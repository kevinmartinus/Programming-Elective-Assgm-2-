...
public class AdminMenuServlet extends HttpServlet {
    ...
    // Loads the current items/categories and shows the management page
    @Override
    ...
    // Applies one of five menu/category edits, chosen by the "action" parameter
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            switch (action) {
                ...
                case "edit" -> menuDAO.updateMenuItem(readMenuItem(request));
    ...
    // Builds a MenuItem from the submitted form fields
    private MenuItem readMenuItem(HttpServletRequest request) {
        ...
        item.setAddonOptions(request.getParameter("addonOptions"));
    ...
    // Parses a form field as an int, falling back to 0 if invalid/missing
    ...
    // Parses a form field as a double, falling back to 0.0 if invalid/missing.
